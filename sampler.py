...
def sample_function(User, user_train_set, Item, usernum, itemnum, Relationships, batch_size, result_queue, SEED,
    ...
    def sample_ii():
        ...
        while True:
            ...
            r_temp1 = list(Item[i]['related'].keys())
            r_temp2 = np.random.choice(len(Item[i]['related'].keys()), 1)
            r_temp = r_temp1[r_temp2[0]]
            #print(invRelationships[r_temp])
            r = invRelationships[r_temp]
